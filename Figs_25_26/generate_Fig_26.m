...
Np=100; Sy=(L1*(L11+1))*Np+Np+Np+(LL1*(L11+1))*Np+Np; 
...
[t,y]=calc(ics,L1*T1+Tgap1,L1*T1+Tgap1+Tgap2,r1,r2,alpha1,a2,alpha3,rho1,rho2,rho3,rho4,epsilon1,gamma1,gamma2,gamma3,mu,V,Np);
...
for i=1:LL1
 ...
 for j=1:L11
     ...
     [t,y]=calc(ics,L1*T1+Tgap1+Tgap2+T11*(j-1)+T1*(i-1),L1*T1+Tgap1+Tgap2+T11*j+T1*(i-1),r1,r2,alpha1,a2,alpha3,rho1,rho2,rho3,rho4,epsilon1,gamma1,gamma2,gamma3,mu,V,Np);
     ics(1)=y(Np,1); ics(2)=y(Np,2); ics(3)=y(Np,3); ics(4)=y(Np,4); ics(5)=y(Np,5)+E0;
     tt(M*Np+1:M*Np+Np)=t;
     Y(M*Np+1:M*Np+Np,:)=y;
     M=M+1;
...
[t,y]=calc(ics,t(length(t)),L1*T1+Tgap1+Tgap2+T1*i,r1,r2,alpha1,a2,alpha3,rho1,rho2,rho3,rho4,epsilon1,gamma1,gamma2,gamma3,mu,V,Np);
tt(M*Np+1:M*Np+Np)=t;
Y(M*Np+1:M*Np+Np,:)=y;
M=M+1;
...
[t,y]=calc(ics,L1*T1+Tgap1+Tgap2+T1*LL1,Tfinal,r1,r2,alpha1,a2,alpha3,rho1,rho2,rho3,rho4,epsilon1,gamma1,gamma2,gamma3,mu,V,Np);
tt(M*Np+1:M*Np+Np)=t;
Y(M*Np+1:M*Np+Np,:)=y;
