...
% End time of the integration and
...
save("TMZ_CAR-T_TMZ_CAR-T_Tsurv_max_r1_0_025_rho4_"+ceil(100*max(rho4val))+"_v_"+v/10^(8)+"_N="+N+".mat");
...
yy0=ics;
options = odeset('RelTol',1e-10,'AbsTol', 1e-10,NonNegative=[1,2,3,4,5]); %,'Events',@events); %, 'AbsTol', 1e-12); %,'AbsTol',1e-13);
...
ics(1)=S10/K; ics(2)=S20/K; ics(3)=RC0/K; ics(4)=0; ics(5)=E0;

Sy=(L1*(L11+1))*Np+Np+Np+(LL1*(L11+1))*Np+Np; 
...
[t,y,te]=calc_event(ics,L1*T1+Tgap1,L1*T1+Tgap1+Tgap2,r1,r2,alpha1,a2,alpha3,rho1,rho2,rho3,rho4,epsilon1,gamma1,gamma2,gamma3,mu,Np,K);
if not(isempty(te)) return; end
...
for i=1:LL1
 ...
 for j=1:L11
     ...
     [t,y,te]=calc_event(ics,L1*T1+Tgap1+Tgap2+T11*(j-1)+T1*(i-1),L1*T1+Tgap1+Tgap2+T11*j+T1*(i-1),r1,r2,alpha1,a2,alpha3,rho1,rho2,rho3,rho4,epsilon1,gamma1,gamma2,gamma3,mu,Np,K);
     if not(isempty(te)) return; end
     ics(1)=y(Np,1); ics(2)=y(Np,2); ics(3)=y(Np,3); ics(4)=y(Np,4); ics(5)=y(Np,5)+E0;
     tt(M*Np+1:M*Np+Np)=t;
     Y(M*Np+1:M*Np+Np,:)=y;
...
[t,y,te]=calc_event(ics,t(length(t)),L1*T1+Tgap1+Tgap2+T1*i,r1,r2,alpha1,a2,alpha3,rho1,rho2,rho3,rho4,epsilon1,gamma1,gamma2,gamma3,mu,Np,K);
if not(isempty(te)) return; end
tt(M*Np+1:M*Np+Np)=t;
Y(M*Np+1:M*Np+Np,:)=y;
M=M+1;
...
[t,y,te]=calc_event(ics,L1*T1+Tgap1+Tgap2+T1*LL1,Tfinal,r1,r2,alpha1,a2,alpha3,rho1,rho2,rho3,rho4,epsilon1,gamma1,gamma2,gamma3,mu,Np,K);
if not(isempty(te)) return; end
tt(M*Np+1:M*Np+Np)=t;
Y(M*Np+1:M*Np+Np,:)=y;
