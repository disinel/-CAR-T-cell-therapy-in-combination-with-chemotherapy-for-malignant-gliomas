...
% Number of stored time points during ODEs integration and total number of points
% in the solution vector
Np=500; Sy=Np+(L1*(L11+1))*Np+Np+Np+(LL1*(L11+1))*Np+Np; 
 
% Loading parameters corresponding to a virtual population of 10,000 patients.
load('parameters_N=10000');

% Number of a patient in the population
n=1454;
...
[tt, Y, t, y] = calc_1cycle(r1,r2,alpha1,a2,alpha3,rho1,rho2,rho3,rho4,epsilon1,gamma1,gamma2,gamma3,mu,V,K,Np,T1,T11,L1,LL1,L11,T2,L2,S10,S20,RC0,E0,Tfinal,Tgap1,Tgap2);
...
% This function computes solution of the governing system of ODE's for the
...
yy0=ics;
options = odeset('RelTol',1e-10,'NormControl','on',NonNegative=[1,2,3,4,5]); %,'Events',@events); %, 'AbsTol', 1e-12); %,'AbsTol',1e-13);
[t,y] = ode89(@func,tspan,yy0,options);
...
function [tt, Y, t, y] = calc_1cycle(r1,r2,alpha1,a2,alpha3,rho1,rho2,rho3,rho4,epsilon1,gamma1,gamma2,gamma3,mu,V,K,Np,T1,T11,L1,LL1,L11,T2,L2,S10,S20,RC0,E0,Tfinal,Tgap1,Tgap2)


Sy=Np+(L1*(L11+1))*Np+Np+Np+(LL1*(L11+1))*Np+Np; 
...
[t,y]=calc(ics,0,Tgap2,r1,r2,alpha1,a2,alpha3,rho1,rho2,rho3,rho4,epsilon1,gamma1,gamma2,gamma3,mu,V,Np);
...
for i=1:L1
 ...
 for j=1:L11
        ...
        [t,y]=calc(ics,Tgap2+T11*(j-1)+T1*(i-1),Tgap2+T11*j+T1*(i-1),r1,r2,alpha1,a2,alpha3,rho1,rho2,rho3,rho4,epsilon1,gamma1,gamma2,gamma3,mu,V,Np);
        ics(1)=y(Np,1); ics(2)=y(Np,2); ics(3)=y(Np,3); ics(4)=y(Np,4); ics(5)=y(Np,5)+E0;
        tt(M*Np+1:M*Np+Np)=t;
        Y(M*Np+1:M*Np+Np,:)=y;
        M=M+1;
...
[t,y]=calc(ics,t(length(t)),Tgap2+T1*i,r1,r2,alpha1,a2,alpha3,rho1,rho2,rho3,rho4,epsilon1,gamma1,gamma2,gamma3,mu,V,Np);
tt(M*Np+1:M*Np+Np)=t;
Y(M*Np+1:M*Np+Np,:)=y;
M=M+1;
...
[t,y]=calc(ics,Tgap2+L1*T1,Tgap2+L1*T1+Tgap1,r1,r2,alpha1,a2,alpha3,rho1,rho2,rho3,rho4,epsilon1,gamma1,gamma2,gamma3,mu,V,Np);
tt(M*Np+1:M*Np+Np)=t;
Y(M*Np+1:M*Np+Np,:)=y;
M=M+1;
...
[t,y]=calc(ics,Tgap2+L1*T1+Tgap1,Tgap2+L1*T1+Tgap1+Tgap2,r1,r2,alpha1,a2,alpha3,rho1,rho2,rho3,rho4,epsilon1,gamma1,gamma2,gamma3,mu,V,Np);
...
for i=1:LL1
 ...
 for j=1:L11
     ...
     [t,y]=calc(ics,Tgap2+L1*T1+Tgap1+Tgap2+T11*(j-1)+T1*(i-1),Tgap2+L1*T1+Tgap1+Tgap2+T11*j+T1*(i-1),r1,r2,alpha1,a2,alpha3,rho1,rho2,rho3,rho4,epsilon1,gamma1,gamma2,gamma3,mu,V,Np);
     ics(1)=y(Np,1); ics(2)=y(Np,2); ics(3)=y(Np,3); ics(4)=y(Np,4); ics(5)=y(Np,5)+E0;
     tt(M*Np+1:M*Np+Np)=t;
     Y(M*Np+1:M*Np+Np,:)=y;
     M=M+1;
...
[t,y]=calc(ics,t(length(t)),Tgap2+L1*T1+Tgap1+Tgap2+T1*i,r1,r2,alpha1,a2,alpha3,rho1,rho2,rho3,rho4,epsilon1,gamma1,gamma2,gamma3,mu,V,Np);
tt(M*Np+1:M*Np+Np)=t;
Y(M*Np+1:M*Np+Np,:)=y;
M=M+1;
...
[t,y]=calc(ics,Tgap2+L1*T1+Tgap1+Tgap2+T1*LL1,Tfinal,r1,r2,alpha1,a2,alpha3,rho1,rho2,rho3,rho4,epsilon1,gamma1,gamma2,gamma3,mu,V,Np);
tt(M*Np+1:M*Np+Np)=t;
Y(M*Np+1:M*Np+Np,:)=y;
