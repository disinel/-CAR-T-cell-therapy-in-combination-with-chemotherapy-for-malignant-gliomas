...
% Fixed parameters
g1=10^(10); g2=10^(10); g3=2*10^(9); v=10^(9); K=5*10^(12); mu=8.32; E0=1;
...
% The time between CAR-T cell applications and the number of injections.  
T2=7; L2=2;
...
% we will map our standard interval for rho_{4}, [a,b] 
% into the interval [a,c]
a=0.002; b=0.1; c=1;

% New range of rho_{4}
rho4val=((c-a)/(b-a)).*rho4val+a*(b-c)/(b-a);

tic

% Main loop that computes survival times for each virtual patient
parfor n=1:N
...
save("CAR-T_trial_max_rho4_"+ceil(100*max(rho4val))+"_v_"+v/10^(8)+"_N="+N+"_");


% This function computes the solution of the governing system of ODEs for  
...
% and returns the time at which 10^{12} is reached, which corresponds  
% to the survival time. Otherwise, the variable 'te' remains empty.
...
% This function follows the CAR-T application protocol for a given virtual  
% patient and returns the patient's survival time. 
