...
% Fixed parameters
g1=10^(10); g2=10^(10); g3=2*10^(9); vtotal=10^(9); K=5*10^(12); mu=8.32; E0=1;
...
% The time interval between CAR-T cell applications.  
T2=7; 
...
% The array of survival times for each patient in the virtual population and each
% value of the number of TMZ cycles.
Q=10; L2q=linspace(1,Q,Q); Tsurv=zeros(Q,N);
...
parfor q=1:Q
    ...
    L2=L2q(q); Tsurv_tmp=zeros(1,N); v=vtotal/L2; V=v/K;

        for n=1:N

            Tc0=T0val(n); fs2=delta2val(n); frc=delta1val(n);  S10=(1-fs2-frc)*Tc0; S20=fs2*Tc0; RC0=frc*Tc0; 

            r1=r1val(n); r2=0.5*r1; alpha1=alpha1val(n);  alpha3=alpha3val(n); rho1=rho1val(n); rho2=rho2val(n); rho3=rho3val(n); rho4=rho4val(n); 
            
            epsilon1=epsilon1val(n);
...
xlabel('L_{2}');
ylabel('$\overline{T}_{surv}$','interpreter','latex');

% Saving generated data
save("CAT-T_only_L2_distributed_dep_N="+N);
